    ...
    def newShape(self):
        ...
        if not self.useDefaultLabelCheckbox.isChecked() or not self.defaultLabelTextLine.text():
            ...
            if self.singleClassMode.isChecked() and self.lastLabel:
                ...
                # Select first label at the start by KAmIL
                if self.nextLabelText == "":
                    self.nextLabelText = self.labelHist[0]
                text = self.labelDialog.popUp(text=self.nextLabelText)
                self.lastLabel = text
        else:
        ...
        if text is not None:
            ...
            if text not in self.labelHist:
                ...
                self.nextLabelText = text
            else:
                self.nextLabelText = self.labelHist[((self.labelHist.index(text) + 1) % len(self.labelHist))]
        else:
